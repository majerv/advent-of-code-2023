...
class Day1Exercise2Test {
  ...
  void testInput(final String input) {
    long count = input.lines().count();
    System.out.println(count);

    long result = exercise.solve(input);
    Assertions.assertEquals(53389, result);
  }

  @ParameterizedTest
  @CsvSource({"two1nine,219"})
  void testDecoding(String encoded, String expected) {
    Assertions.assertEquals(expected, exercise.resolveNumbers(encoded));
  }

  @ParameterizedTest
  @CsvSource({"enin1owt,912"})
  void testReverseDecoding(String encoded, String expected) {
    Assertions.assertEquals(expected, exercise.resolveNumbersReverse(encoded));
  }

  @ParameterizedTest
  @CsvSource({
    "9seventhreefourzlqgjnrmq7xcmntjncntsixthlgq,96",
    "dktgqszxcptlpxpzlsix8onetwoqlxk,62",
    "zbjbeightfhbhcfkbhrthqhgsixthree11five,85",
    "hldmdhr4,44",
    "4jvpzjcrm8,48",
    "5ncdmtdhbs,55",
    "fjshmr65sixrkqbtpxqnpqgjthplnlv7dfrktxvng, 67",
    "nineldnhvqqdrt2sevenqxgltrhgvtdnfcxcsknsgkzjzbvc, 97",
    "nffone49, 19",
    "two7fourvstgmr3rtlztqddmnghm, 23",
    "167, 17",
    "7j, 77",
    "7, 77",
    "'', 0",
  ...
  void testFullDecoding(String encoded, int expected) {
    Assertions.assertEquals(expected, exercise.lineToCalibrationValue(encoded));
  }
}
